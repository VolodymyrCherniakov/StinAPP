...
def get_stock_entry(ticker, use_cache_only=False):
    with cache_lock:
...
def get_stocks():
    ...
    if not tickers:
        return jsonify({})
    return jsonify({ticker: get_stock_entry(ticker, use_cache_only=True) for ticker in tickers})
...
# Scheduler pro budoucí použití (zatím bez funkce)
scheduler = BackgroundScheduler()
trigger = CronTrigger(hour='0,6,12,18', minute=0)
# scheduler.add_job(func=scheduled_stock_fetch, trigger=trigger)  # Pokud bys měl DEFAULT_TICKERS
scheduler.start()
